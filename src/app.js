...
let walletStatusMessage =
  "Connect a Tezos browser wallet to collect your current DRUM drop.";
let mintStatusMessage = "Pick a drop and connect your wallet.";
...
function renderScoreboard() {
  ...
  elements.comboCaption.textContent =
    combo > 0
      ? `Combo alive. Next hit is worth ${nextGain} DRUM.`
      : `Tap to warm up the groove. Next hit is worth ${nextGain} DRUM.`;
}
...
function renderCollection() {
  ...
  elements.collectionStatus.textContent = nextDrop
    ? `${formatCount(nextDrop.threshold - state.profile.lifetimeTokens)} DRUM until ${nextDrop.title}.`
    : "Every drop is unlocked. The room is fully loaded.";
...
async function connectWallet() {
  ...
  try {
    ...
    walletStatusMessage =
      "Open your Tezos browser wallet, approve the pairing request, and then confirm the connection.";
    renderWalletState();
