...
var GALLERY_STORAGE_KEY = "industrynext.nouns.gallery";
...
var mintBusy = false;
var walletStatusMessage = "Bundled Beacon + Taquito wallet flow for the official Tezos tutorial contract on Shadownet.";
var mintStatusMessage = "Connect a Tezos wallet to mint this noun.";
...
function syncMintDraft(noun, draft = {}) {
  ...
  elements.tokenDescription.value = draft.description?.trim() || buildDefaultDescription(noun);
}
...
function renderGallery() {
  ...
  elements.galleryList.innerHTML = galleryEntries.map((entry) => {
...
async function ensureTezosToolkit() {
  if (!tezosClientPromise) {
    tezosClientPromise = import("./chunks/tezos-client-6ZUOYADT.js");
  }
...
async function mintCurrentNoun() {
  ...
  renderWalletState();
  try {
    const { MichelsonMap: MetadataMap, stringToBytes: toBytes, tezosToolkit: toolkit } = await ensureTezosToolkit();
