...
var ROOM_CACHE_STORAGE_KEY = "drum-nouns-jam.rooms.v1";
var DEFAULT_ROOM_ID = "global-jam";
var COMBO_TIMEOUT_MS = 2400;
var ROOM_POLL_MS = 12e3;
var ROOM_HEARTBEAT_MS = 12e3;
var DIRECTORY_POLL_MS = 18e3;
var ROOM_STREAM_RETRY_MS = 2600;
var PULSE_FLUSH_DELAY_MS = 900;
var ROOM_CACHE_TTL_MS = 7 * 24 * 60 * 60 * 1e3;
var ROOM_CACHE_LIMIT = 16;
var CROWD_STAGE_SLOTS = [
  { x: 18, y: 26 },
  { x: 82, y: 26 },
  { x: 16, y: 74 },
  { x: 84, y: 74 },
...
var ROOM_GOAL_MILESTONES = [2, 3, 5];
var elements = {
...
var supportJumpLinks = [...document.querySelectorAll("[data-support-jump]")];
var supportPanels = [...document.querySelectorAll("[data-support-panel]")];
var supportToggleButtons = [...document.querySelectorAll("[data-support-toggle]")];
var imageData;
var tezosSdkPromise;
var tezosSdk;
var tezosToolkit;
...
var mintBusy = false;
var walletStatusMessage = "";
var mintStatusMessage = "";
var mintResultMarkup = "";
var imageDataLoaded = false;
var pulseFlushTimer;
var roomPollTimer;
var roomHeartbeatTimer;
var roomDirectoryTimer;
var roomEventSource;
var roomStreamRetryTimer;
var comboTimer;
var pulseInFlight = false;
var presenceInFlight = false;
var nameSyncTimer;
var appearanceSyncTimer;
var supportPanelObserver;
var drumReturnObserver;
...
function buildDailySpotlightRoomId(date = /* @__PURE__ */ new Date()) {
...
function formatSecondsLeft(durationMs) {
  return `${Math.max(1, Math.ceil(durationMs / 1e3))}s`;
}
...
function sanitizeRoomDirectory(directory) {
  ...
  return {
    ...
    rooms: Array.isArray(directory.rooms) ? directory.rooms.map((room) => ({
      ...
      topPlayerSeed: room?.topPlayerSeed && typeof room.topPlayerSeed === "object" ? normalizeStoredSeed(room.topPlayerSeed) : null,
...
function sanitizeStoredRoomSnapshot(snapshot, expectedRoomId) {
  ...
  return {
    ...
    players: Array.isArray(snapshot.players) ? snapshot.players.slice(0, 24).map((player) => ({
    ...
    events: Array.isArray(snapshot.events) ? snapshot.events.slice(0, 18).map((event, index) => ({
...
function loadRoomCacheEntries() {
  try {
    ...
    return Object.entries(parsed).map(([roomId, entry]) => {
    ...
    }).filter(Boolean).sort((left, right) => right[1].savedAt - left[1].savedAt).slice(0, ROOM_CACHE_LIMIT);
  } catch {
    return [];
  }
}
...
function createFilenameStem() {
  const context = buildMintContext();
  const targetStem = context.target === MINT_TARGETS.NOUN ? `${context.target}-${context.noun.labels.head}` : `${context.target}-${context.drop.id}`;
  return sanitizeText(
...
function computeGainForHit(comboValue = getCurrentCombo() + 1, options = {}) {
  ...
  const powerHitIndex = activePowerDrop ? options.powerHitIndex ?? state.power.hitCounter + 1 : 0;
  let flatBonus = 0;
  let multiplier = 1;
  let triggerLabel = "";
...
function buildPowerStatusNote(drop, status, breakdown = null) {
  ...
  if (status.kind === "live") {
    switch (drop.id) {
      case "laser-tom":
        return breakdown?.flatBonus >= 4 ? `Laser Burst is live and the next hit is primed for +4 DRUM.` : `Laser Burst is live. Every 4th hit fires +4 DRUM for ${formatSecondsLeft(status.remainingMs)}.`;
      case "street-parade":
...
function renderScoreboard() {
  ...
  elements.comboCaption.textContent = combo > 0 ? `Combo alive. Next hit is worth ${nextGain.total} DRUM. ${buildPowerStatusNote(selectedDrop, powerStatus, nextGain)}` : `Tap to warm up the groove. Next hit is worth ${nextGain.total} DRUM. ${buildPowerStatusNote(selectedDrop, powerStatus, nextGain)}`;
}
function renderPowerUpPanel() {
  ...
  elements.activatePowerUpButton.textContent = status.kind === "ready" ? selectedDrop.power.activateLabel : status.kind === "live" ? `${formatSecondsLeft(status.remainingMs)} live` : status.kind === "cooldown" ? `Ready in ${formatSecondsLeft(status.remainingMs)}` : `Unlock at ${formatCount(selectedDrop.threshold)}`;
  ...
  elements.powerUpNote.textContent = status.kind === "ready" ? `Next hit is ${nextGain.total} DRUM before you fire it. ${selectedDrop.power.short}.` : buildPowerStatusNote(selectedDrop, status, nextGain);
}
function getStagePlayers(room = state.room) {
  const players = room.players || [];
  const featured = players.filter((player) => player.id !== state.profile.playerId).sort((left, right) => {
...
function computeJamIntensity() {
  ...
  return Math.max(
    ...
    Math.min(
      100,
      10 + (metrics.activeCount || 0) * 16 + (metrics.recentReactions || 0) * 8 + (metrics.syncedBursts || 0) * 6
    )
  );
}
...
function renderTrafficBoard() {
  ...
  elements.trafficNote.textContent = liveRooms > 0 ? "The daily spotlight rotates every day, and the busiest rooms float to the top so people can pile into the same jam." : "No loud rooms yet. Global jam and the daily spotlight are ready if you want to start the crowd.";
  if (!visibleRooms.length) {
    elements.trafficRoomList.innerHTML = '<div class="empty-state">Open a room, share it, or jump into the daily spotlight to start the first crowd.</div>';
    return;
  }
  elements.trafficRoomList.innerHTML = visibleRooms.map((room) => {
    const isCurrent = room.roomId === state.roomId;
    const roomLeadNoun = room.topPlayerSeed ? buildNoun(room.topPlayerSeed) : isCurrent ? getCurrentNoun() : null;
    const trafficPulse = Math.max(
      isCurrent ? 22 : 10,
      Math.min(
        100,
        room.activeCount * 24 + room.recentReactions * 10 + room.syncedBursts * 10 + Math.round(room.totalTokens / 20)
      )
    );
    const trafficBadge = room.activeCount > 0 ? `${room.activeCount} live` : room.participantCount > 0 ? `${room.participantCount} seen` : "Fresh room";
    const trafficCopy = room.activeCount > 0 ? `${formatCount(room.totalTokens)} DRUM \u2022 crew ${formatMultiplier(room.crewMultiplier)} \u2022 ${room.topPlayerName || "First drummer"} on top` : room.participantCount > 0 ? `${formatCount(room.participantCount)} drummers have touched this room \u2022 best ${formatCount(room.highestCombo)}x` : "Daily spotlight and global rooms are the best places to start a crowd.";
    const trafficMeta = room.latestEventMessage ? trimText(room.latestEventMessage, "The tape is fresh.", 96) : room.roomId === spotlightRoomId ? "This is the shared room of the day." : room.roomId === DEFAULT_ROOM_ID ? "Always-on room for anyone dropping in." : "Jump in and make the first noise here.";
    return `
...
function renderCollabExperience() {
  ...
  elements.crowdStage.innerHTML = stagePlayers.map((player, index) => {
...
function renderJamGuide() {
  ...
  if (nextDrop) {
    ...
    elements.guideUnlockCopy.textContent = `${formatCount(remaining)} DRUM to unlock ${nextDrop.power.name}. ${nextDrop.power.short}.`;
    ...
    elements.guideUnlockCopy.textContent = powerStatus.kind === "ready" ? `${selectedDrop.power.short}. Your next hit is ${nextGain.total} DRUM before you fire it.` : buildPowerStatusNote(selectedDrop, powerStatus, nextGain);
  ...
  if (!leader || rankedPlayers.length <= 1) {
    ...
    elements.guideRaceCopy.textContent = "You own the board right now. Share the room and turn the solo run into a chase.";
    ...
    elements.guideRaceCopy.textContent = `${runnerUp?.name || "The room"} is ${formatCount(lead)} DRUM behind you. Keep the pad warm.`;
    ...
    elements.guideRaceCopy.textContent = `${formatCount(gap)} DRUM to pass ${chaseTarget?.name || "the leader"} and climb the board.`;
  ...
  if (activeCount <= 1) {
    ...
    elements.guideRoomCopy.textContent = knownPlayers > 1 ? `This room has history. Share ${state.roomId} and pull one more noun back on stage.` : "One more live noun wakes the room up and makes the shared groove feel bigger.";
    ...
    elements.guideRoomCopy.textContent = `${needed} more noun${needed === 1 ? "" : "s"} needed. Share the room and keep the crowd meter climbing.`;
    ...
    elements.guideRoomCopy.textContent = `${activeCount} live nouns are on stage. Throw a boom drop and keep the feed moving.`;
...
function renderCollection() {
  ...
  elements.collectionStatus.textContent = nextDrop ? `${formatCount(nextDrop.threshold - state.profile.lifetimeTokens)} DRUM until ${nextDrop.title}. Each unlock adds a new power-up.` : "Every drop is unlocked. Swap powers whenever the room changes.";
  elements.collectionPanelPreview.textContent = nextDrop ? `Next: ${nextDrop.title} in ${formatCount(nextDrop.threshold - state.profile.lifetimeTokens)} DRUM` : `All drops unlocked \u2022 ${currentDrop.title} equipped`;
  ...
  elements.unlockShelf.innerHTML = DROP_LIBRARY.map((drop) => {
    ...
    const previewNoun = isSelected ? getCurrentNoun() : buildNoun(buildSeedVariant(state.profile.seed, `${drop.id}:${drop.tier}:${drop.threshold}`));
    const previewBreakdown = isSelected ? computeGainForHit(getCurrentCombo(now) + 1, { now }) : null;
    const buttonLabel = !unlockedState ? `Need ${formatCount(drop.threshold)} DRUM` : isSelected ? "Equipped now" : "Equip power-up";
    return `
      <article
...
function renderMintControls() {
  ...
  elements.mintedAssetStatus.textContent = mintedHash ? `Latest ${context.mintedLabel} mint: ${compactAddress(mintedHash)}` : `No ${context.mintedLabel} mint saved yet on this device.`;
  ...
  elements.mintPanelPreview.textContent = connectedAddress ? `${compactAddress(connectedAddress)} \u2022 ${context.buttonLabel}` : `Wallet not connected \u2022 ${context.pillLabel}`;
...
function renderLeaderboard() {
  ...
  if (!leader || rankedPlayers.length === 1) {
    elements.leaderboardStatus.textContent = "You own the board right now. Share the room to turn this into a race.";
  } else if (selfPlayer?.rank === 1) {
  ...
  elements.leaderboardPanelPreview.textContent = leader ? `${leader.name}${leader.id === state.profile.playerId ? " (you)" : ""} leads with ${formatCount(leader.totalTokens)} DRUM` : "No leaderboard data yet";
  elements.leaderboardGlanceGap.textContent = !leader || rankedPlayers.length === 1 ? "Solo" : selfPlayer?.rank === 1 ? `Ahead ${formatCount(Math.max(0, (selfPlayer?.totalTokens || 0) - (runnerUp?.totalTokens || 0)))}` : `Need ${formatCount(Math.max(0, (leader.totalTokens || 0) - (selfPlayer?.totalTokens || 0)))}`;
  ...
  elements.leaderboardList.innerHTML = visiblePlayers.map((player) => {
    const noun = buildNoun(player.seed);
    ...
    const progressPercent = leader ? clampPercent(player.totalTokens / Math.max(1, leader.totalTokens) * 100, isSelf ? 28 : 12) : 100;
    ...
    const leaderboardDetail = player.rank === 1 ? rankedPlayers.length > 1 ? `Ahead by ${formatCount(Math.max(0, player.totalTokens - (runnerUp?.totalTokens || 0)))}` : "Solo set" : `${formatCount(Math.max(0, (leader?.totalTokens || 0) - player.totalTokens))} behind`;
    return `
        <article
...
function renderCrew() {
  ...
  elements.crewPanelPreview.textContent = activeCount > 0 ? `${activeCount} active \u2022 crew at ${formatMultiplier(getCrewMultiplier())}` : `No live crew in ${state.roomId} yet`;
...
function renderFeed() {
  ...
  elements.socialPanelPreview.textContent = latestEvent ? `${latestEvent.playerName}: ${trimText(latestEvent.message, "Fresh room energy.", 72)}` : "No room moments yet";
...
function triggerCollaborativeMoments(previousRoom, nextRoom) {
  ...
  unseenEvents.forEach((event) => {
    ...
    const label = event.type === "join" ? `${event.playerName} joined` : `${event.playerName} ${event.reaction || "HYPE"}`;
    spawnGainBurst(0, {
      label,
      left: slot.x,
      top: slot.y,
...
function roomHasSavedActivity(room = state.room) {
  return Boolean(
    room?.totals?.tokens || room?.totals?.hits || room?.players?.length || room?.events?.length || room?.sequence
  );
}
...
function closeRoomStream() {
  ...
  roomStreamRetryTimer = void 0;
  if (!roomEventSource) {
    return;
  }
  roomEventSource.close();
  roomEventSource = void 0;
}
...
async function joinRoom() {
  ...
  try {
    ...
    state.shareStatus = roomHasSavedActivity() ? `Using saved room history while sync is paused: ${formatError(error)}` : `Room sync paused: ${formatError(error)}`;
    renderAll();
  } finally {
...
async function syncRoom() {
  try {
    ...
    state.shareStatus = roomHasSavedActivity() ? `Showing saved room history while live sync retries: ${formatError(error)}` : `Retrying room sync: ${formatError(error)}`;
    renderAll();
  }
}
...
async function restoreWalletSession() {
  try {
    const { createTezosWallet, detectTezosExtensions, getActiveWalletAccount, tezosToolkit: toolkit } = await ensureTezosToolkit();
...
async function connectWallet() {
  ...
  try {
    ...
    walletInstance = createTezosWallet(buildTezosWalletOptions());
    walletStatusMessage = extensions.length === 1 ? `Open ${extensions[0].name}, then approve the Beacon connection request.` : "Choose a Tezos wallet extension, then approve the Beacon connection request.";
    renderWalletState();
