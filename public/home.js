...
function computeRoomStats(room) {
  ...
  const hypeIndex = Math.round(
    room.metrics.activeCount * 14 + room.metrics.recentReactions * 11 + room.metrics.syncedBursts * 13 + room.metrics.crewMultiplier * 20
  );
  const pulseVelocity = room.metrics.activeCount > 0 ? room.totals.tokens / Math.max(1, room.metrics.activeCount) : room.totals.tokens;
