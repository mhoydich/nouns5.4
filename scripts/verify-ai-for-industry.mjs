...
assert.match(earlyCareerHtml, /Seven useful doors/);
assert.match(earlyCareerHtml, /AI FOR INDUSTRY \/ 10 PARTS/);
assert.match(earlyCareerHtml, /href="\/early-career\/ai-for-industry\/"/);
assert.equal((earlyCareerHtml.match(/class="resource-card/g) || []).length, 7);
assert.equal(earlyCareer.resources.length, 7);
