...
assert.equal((html.match(/class="market-card/g) || []).length, 13);
assert.equal((html.match(/class="opportunity-card/g) || []).length, 4);
assert.equal((html.match(/data-opportunity/g) || []).length, 5);
...
assert.equal((html.match(/data-kind="task"/g) || []).length, 5);
...
assert.equal(data.counts.first_tasks, 5);
...
assert.equal(data.opportunities.length, 9);
